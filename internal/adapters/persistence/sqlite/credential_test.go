...
func newCredentialVault(t *testing.T, db *sql.DB) *sqlite.CredentialVault {
	t.Helper()
	return sqlite.NewCredentialVault(db, testCipher(t), fixedClock{t: time.Unix(1700000000, 0).UTC()})
}
...
func TestCredentialVaultEncryptedAtRest(t *testing.T) {
	...
	if err := rdb.QueryRowContext(ctx,
		`SELECT secret_sealed, creditor_key_sealed FROM bank_credentials WHERE tenant_id = ? AND bank_id = ?`,
