...
// TestSQLiteWebhookRefSupersede proves PutWebhookRef has SUPERSEDE semantics (B1): a
...
func TestSQLiteWebhookRefSupersede(t *testing.T) {
	t.Parallel()
...
// TestSQLiteWebhookRefRevoke proves RevokeWebhookRefs soft-deletes the tenant's active
...
func TestSQLiteWebhookRefRevoke(t *testing.T) {
	t.Parallel()
	db, s := newWebhookRefStore(t, &akClock{t: time.Unix(0, 0).UTC()}, "tenant-1")
	ctx := context.Background()
...
// TestSQLiteWebhookRefRevokeValidation rejects an empty tenant id.
func TestSQLiteWebhookRefRevokeValidation(t *testing.T) {
	t.Parallel()
...
// TestSQLiteWebhookRefSupersedeIsolation proves superseding one tenant's ref never
// revokes another tenant's active ref.
func TestSQLiteWebhookRefSupersedeIsolation(t *testing.T) {
	t.Parallel()
...
// TestSQLiteWebhookRefReactivateOnRePut proves re-putting a PREVIOUSLY REVOKED hash
// re-activates it (ON CONFLICT clears revoked_at) — a defensive edge; a fresh 256-bit
// ref never actually collides.
func TestSQLiteWebhookRefReactivateOnRePut(t *testing.T) {
	t.Parallel()
	_, s := newWebhookRefStore(t, &akClock{t: time.Unix(0, 0).UTC()}, "tenant-1")
	ctx := context.Background()
...
func TestMigration0017Reversible(t *testing.T) {
	...
	hasRevokedCol := func() bool {
		rows, err := db.QueryContext(context.Background(), `PRAGMA table_info(webhook_tenant_refs)`)
		if err != nil {
