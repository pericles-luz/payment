...
func newCertificateVault(t *testing.T, db *sql.DB) *sqlite.CertificateVault {
	t.Helper()
	return sqlite.NewCertificateVault(db, testCipher(t), fixedClock{t: time.Unix(1700000000, 0).UTC()})
}
...
func TestCertificateVaultKeyEncryptedAtRest(t *testing.T) {
	...
	if err := rdb.QueryRowContext(ctx,
		`SELECT key_pem_sealed, cert_pem FROM bank_certificates WHERE tenant_id = ? AND bank_id = ?`,
