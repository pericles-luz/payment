...
func newWebhookVault(t *testing.T, db *sql.DB) *sqlite.OutboundWebhookVault {
	t.Helper()
	return sqlite.NewOutboundWebhookVault(db, testCipher(t), fixedClock{t: time.Unix(1700000000, 0).UTC()})
}
...
func TestOutboundWebhookVaultSecretCiphertextAtRest(t *testing.T) {
	...
	var sealed []byte
	if err := db.QueryRowContext(ctx, `SELECT signing_secret_sealed FROM account_outbound_webhook WHERE account_id = ?`, "acct-1").Scan(&sealed); err != nil {
...
func TestOutboundWebhookVaultAADRowBinding(t *testing.T) {
	...
	var sealed []byte
	if err := db.QueryRowContext(ctx, `SELECT signing_secret_sealed FROM account_outbound_webhook WHERE account_id = ?`, "acct-A").Scan(&sealed); err != nil {
...
func TestOutboundWebhookVaultWrongKEKFailsClosed(t *testing.T) {
	...
	wrong := sqlite.NewOutboundWebhookVault(db, otherCipher(t), fixedClock{t: time.Unix(1700000000, 0).UTC()})
