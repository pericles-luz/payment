...
func (r repo) FindRecByID(ctx context.Context, tenantID, idRec string) (*recurrence.Rec, error) {
	row := r.q.QueryRowContext(ctx,
		`SELECT id_rec, tenant_id, bank_id, contrato, devedor_doc, devedor_nome,
		    data_inicial, periodicidade, valor_cents, status, loc_id, jornada_txid, created_at, updated_at
		 FROM pix_rec WHERE tenant_id = ? AND id_rec = ?`, tenantID, idRec)
	...
	// The QR-journey columns are nullable (0019): a mandate registered before that
	// migration, or one activated through solicrec, legitimately has neither. They scan
	// through sql.Null* so a NULL rehydrates as the zero binding instead of erroring.
...
func (r repo) FindCobRByTxID(ctx context.Context, tenantID, txID string) (*recurrence.CobR, error) {
	row := r.q.QueryRowContext(ctx,
		`SELECT tx_id, id_rec, tenant_id, vencimento, valor_cents, status, created_at, updated_at
		 FROM pix_cobr WHERE tenant_id = ? AND tx_id = ?`, tenantID, txID)
	c, err := scanCobR(row)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, shared.ErrNotFound
...
func (s *Store) ListCobRByRec(ctx context.Context, tenantID, idRec string) ([]*recurrence.CobR, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT tx_id, id_rec, tenant_id, vencimento, valor_cents, status, created_at, updated_at
		 FROM pix_cobr WHERE tenant_id = ? AND id_rec = ? ORDER BY vencimento ASC, tx_id ASC`,
		tenantID, idRec)
	if err != nil {
