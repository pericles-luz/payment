...
// newStoreDSN opens a fresh migrated SQLite store backed by a temp file and returns
...
// newAuditStore.
func newStoreDSN(t *testing.T) (*sqlite.Store, string) {
	t.Helper()
...
func ledgerAccountIsNull(t *testing.T, dsn, id string) bool {
	...
	if err := db.QueryRowContext(context.Background(),
		`SELECT account_id FROM billing_ledger WHERE id = ?`, id).Scan(&acct); err != nil {
		t.Fatalf("read account_id for %s: %v", id, err)
	}
	return !acct.Valid
}

// TestSQLiteLedgerStampsAccount is the metering acceptance (SIN-69127): a ledger
...
func TestSQLiteLedgerStampsAccount(t *testing.T) {
	t.Parallel()
	s, dsn := newStoreDSN(t)
...
// TestSQLiteListLedgerEntriesByAccount is the rollup read acceptance: an account's
// entries are returned across ALL of its tenants, newest-first, and never leak
// another account's usage.
func TestSQLiteListLedgerEntriesByAccount(t *testing.T) {
	t.Parallel()
	s := newStore(t)
	ctx := context.Background()
...
// TestSQLiteAuditStampsAccount checks the audit adapter persists the entry's derived
// account attribution (SIN-69127) into audit_log.account_id, so the forensic trail
// completes the account→tenant rollup.
func TestSQLiteAuditStampsAccount(t *testing.T) {
	t.Parallel()
	s, dsn := newStoreDSN(t)
	ctx := context.Background()
...
func TestMigration0009Reversible(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	db, err := sqlite.Open(filepath.Join(t.TempDir(), "m0009rev.db"))
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	defer func() { _ = db.Close() }()
