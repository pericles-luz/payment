...
func newConsentStore(t *testing.T) *sqlite.Store {
	t.Helper()
	dsn := filepath.Join(t.TempDir(), "consent.db")
...
func TestSQLiteConsentRoundTrip(t *testing.T) {
	s := newConsentStore(t)
...
func TestSQLiteConsentNotFound(t *testing.T) {
	s := newConsentStore(t)
	ctx := context.Background()
...
// TestSQLiteConsentTenantIsolation proves one tenant can never read another's
// consent — the same (subject, version) owned by tenant-a is invisible to
// tenant-b (threat P1, no cross-tenant oracle).
func TestSQLiteConsentTenantIsolation(t *testing.T) {
	s := newConsentStore(t)
	ctx := context.Background()
	at := time.Date(2026, 7, 3, 12, 0, 0, 0, time.UTC)
...
// TestSQLiteConsentAppendOnlyImmutable is the acceptance proof: re-consent to the
...
func TestSQLiteConsentAppendOnlyImmutable(t *testing.T) {
	s := newConsentStore(t)
	ctx := context.Background()
...
// TestSQLiteConsentReusedIDRejected proves the primary-key immutability guard: an
...
func TestSQLiteConsentReusedIDRejected(t *testing.T) {
	s := newConsentStore(t)
	ctx := context.Background()
	at := time.Date(2026, 7, 3, 12, 0, 0, 0, time.UTC)
...
// TestSQLiteConsentFindLatestAcrossVersions confirms FindLatest is scoped to the
// requested version, not merely the newest row for the subject.
func TestSQLiteConsentFindLatestAcrossVersions(t *testing.T) {
	s := newConsentStore(t)
	ctx := context.Background()
