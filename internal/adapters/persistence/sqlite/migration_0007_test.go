...
func legacyFS(t *testing.T) fs.FS {
	...
	for _, e := range entries {
		name := e.Name()
		if !strings.HasSuffix(name, ".up.sql") || strings.HasPrefix(name, "0007_") {
			continue
		}
...
func TestMigration0007BackfillSelfAccount(t *testing.T) {
	...
	if err := sqlite.Migrate(ctx, db, legacyFS(t)); err != nil {
	...
	if got := queryInt(t, db, `SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND name='ix_ledger_account_at'`); got != 1 {
...
func TestMigration0007Reversible(t *testing.T) {
	...
	// Full schema (through 0007) with a seeded tenant so backfill has work to do.
	...
	if got := queryInt(t, db, `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='accounts'`); got != 1 {
		t.Fatalf("accounts table should exist after up, got %d", got)
	}

	// Down.
	mustExec(t, db, readMigration(t, "0007_two_level_tenancy.down.sql"))
	if got := queryInt(t, db, `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='accounts'`); got != 0 {
		t.Fatalf("accounts table should be dropped after down, got %d", got)
	}
	if got := queryInt(t, db, `SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND name='ix_ledger_account_at'`); got != 0 {
