...
func newWebhookRefStore(t *testing.T, clock *akClock, tenantIDs ...string) (*sql.DB, *sqlite.WebhookRefStore) {
	t.Helper()
	...
	return db, sqlite.NewWebhookRefStore(db, clock)
}
...
// TestSQLiteWebhookRefPutLookup is the happy path: a minted ref persists and resolves
// to its tenant after being written durably.
func TestSQLiteWebhookRefPutLookup(t *testing.T) {
	t.Parallel()
...
// TestSQLiteWebhookRefHashAtRest proves the plaintext ref is NEVER written: the stored
// ref_sha256 equals hex(sha256(ref)) and no row contains the plaintext.
func TestSQLiteWebhookRefHashAtRest(t *testing.T) {
	t.Parallel()
	...
	if err := db.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM webhook_tenant_refs WHERE ref_sha256 = ?`, ref).Scan(&hit); err != nil {
		t.Fatalf("scan: %v", err)
	}
	if hit != 0 {
	...
	var stored string
	if err := db.QueryRowContext(ctx,
		`SELECT ref_sha256 FROM webhook_tenant_refs WHERE tenant_id = ?`, "tenant-1").Scan(&stored); err != nil {
...
// TestSQLiteWebhookRefUnknownMiss proves an unregistered ref is a non-oracle miss
// (ok=false, err=nil) — indistinguishable from any other miss.
func TestSQLiteWebhookRefUnknownMiss(t *testing.T) {
	t.Parallel()
	_, s := newWebhookRefStore(t, &akClock{t: time.Unix(0, 0).UTC()}, "tenant-1")
	ctx := context.Background()
...
// TestSQLiteWebhookRefIsolation proves one tenant's ref never resolves to another.
func TestSQLiteWebhookRefIsolation(t *testing.T) {
	t.Parallel()
...
// TestSQLiteWebhookRefForeignKey proves a ref can only be bound to a real tenant.
func TestSQLiteWebhookRefForeignKey(t *testing.T) {
	t.Parallel()
...
// TestSQLiteWebhookRefValidation rejects an empty hash or tenant id.
func TestSQLiteWebhookRefValidation(t *testing.T) {
	t.Parallel()
	_, s := newWebhookRefStore(t, &akClock{t: time.Unix(0, 0).UTC()}, "tenant-1")
	ctx := context.Background()
	ref, _ := webhookref.Generate()
...
// TestSQLiteWebhookRefDuplicateIsNoop proves re-putting the SAME ref hash is idempotent
// (INSERT OR IGNORE), not an error on the unique index.
func TestSQLiteWebhookRefDuplicateIsNoop(t *testing.T) {
	t.Parallel()
	_, s := newWebhookRefStore(t, &akClock{t: time.Unix(0, 0).UTC()}, "tenant-1")
	ctx := context.Background()
	ref, _ := webhookref.Generate()
...
// TestSQLiteWebhookRefLookupReadError proves a read against a DROPPED table surfaces a
// non-nil error (fail-closed at the authenticator), distinct from a benign miss.
func TestSQLiteWebhookRefLookupReadError(t *testing.T) {
	t.Parallel()
	db, s := newWebhookRefStore(t, &akClock{t: time.Unix(0, 0).UTC()}, "tenant-1")
	ctx := context.Background()
...
func TestMigration0016Reversible(t *testing.T) {
	...
	if n := queryInt(t, db, `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='webhook_tenant_refs'`); n != 1 {
	...
	if n := queryInt(t, db, `SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='webhook_tenant_refs'`); n != 0 {
