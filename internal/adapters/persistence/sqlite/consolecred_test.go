...
var (
	_ app.ConsoleCredentialStore = (*sqlite.ConsoleCredentialVault)(nil)
	_ app.TOTPReplayStore        = (*sqlite.ConsoleReplayStore)(nil)
)
...
func TestConsoleCredentialVaultRoundTripSurvivesRestart(t *testing.T) {
	...
	v1 := sqlite.NewConsoleCredentialVault(db, testCipher(t), clk)
	...
	v2 := sqlite.NewConsoleCredentialVault(db2, testCipher(t), clk)
...
func TestConsoleCredentialVaultSaveUpsert(t *testing.T) {
	_, db := openVaultDB(t)
	clk := fixedClock{t: time.Unix(1700000000, 0).UTC()}
	v := sqlite.NewConsoleCredentialVault(db, testCipher(t), clk)
...
func TestConsoleCredentialVaultOpenFailsWithWrongKey(t *testing.T) {
	...
	v2 := sqlite.NewConsoleCredentialVault(db, otherCipher(t), clk)
...
func TestConsoleReplayStoreRoundTripSurvivesRestart(t *testing.T) {
	dsn, db := openVaultDB(t)
	clk := fixedClock{t: time.Unix(1700000000, 0).UTC()}
	r1 := sqlite.NewConsoleReplayStore(db, clk)
	...
	t.Cleanup(func() { _ = db2.Close() })
	r2 := sqlite.NewConsoleReplayStore(db2, clk)
...
func buildConsoleService(t *testing.T, db *sql.DB, clk fixedClock) *app.ConsoleAuthService {
	t.Helper()
	creds := sqlite.NewConsoleCredentialVault(db, testCipher(t), clk)
	replay := sqlite.NewConsoleReplayStore(db, clk)
