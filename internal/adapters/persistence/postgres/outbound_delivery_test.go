...
func TestOutboundDeliveryRoundTripSurvivesRestart(t *testing.T) {
	...
	if err := postgres.NewOutboundDeliveryStore(db).EnqueueDelivery(ctx, newDelivery(t, "d1", "acct-verz", "ten-1", "ek-1", "tx-1")); err != nil {
	...
	got, err := postgres.NewOutboundDeliveryStore(db2).PendingDeliveries(ctx, "acct-verz")
	if err != nil {
...
func TestOutboundDeliveryIdempotentEnqueue(t *testing.T) {
	...
	_, db := openVaultDB(t)
	ctx := context.Background()
	s := postgres.NewOutboundDeliveryStore(db)
