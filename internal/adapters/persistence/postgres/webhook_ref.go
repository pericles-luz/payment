...
func (s *WebhookRefStore) PutWebhookRef(ctx context.Context, refSHA []byte, tenantID string) error {
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE webhook_tenant_refs SET revoked_at = $1
		 WHERE tenant_id = $2 AND revoked_at IS NULL AND ref_sha256 <> $3`,
...
func (s *WebhookRefStore) LookupWebhookRef(ctx context.Context, refSHA []byte) (string, bool, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT tenant_id FROM webhook_tenant_refs WHERE ref_sha256 = $1 AND revoked_at IS NULL`,
