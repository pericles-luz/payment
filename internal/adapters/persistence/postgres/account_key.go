...
func (s *AccountKeyStore) PutKey(ctx context.Context, accountID string) (string, error) {
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE account_keys SET active = 0, rotated_at = $1 WHERE account_id = $2 AND active = 1`,
...
func (s *AccountKeyStore) AuthenticateAccountKey(ctx context.Context, secret string) (string, bool) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT account_id FROM account_keys WHERE key_hash = $1 AND active = 1`, hash).Scan(&accountID)
	if err != nil {
