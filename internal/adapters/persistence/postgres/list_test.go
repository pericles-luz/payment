...
func seedTenants(t *testing.T, s *postgres.Store, ids ...string) {
	t.Helper()
	for i, id := range ids {
...
func TestPostgresListTenants(t *testing.T) {
	t.Parallel()
	s := newStore(t)
...
func TestPostgresListEndpointPrices(t *testing.T) {
	t.Parallel()
	s := newStore(t)
	ctx := context.Background()
...
func TestPostgresListLedgerEntries(t *testing.T) {
	t.Parallel()
	s := newStore(t)
	ctx := context.Background()
	seedTenants(t, s, "t1", "t2")
