...
func newRecStore(t *testing.T) (*postgres.Store, string) {
	t.Helper()
	dsn := testDSN(t)
...
func TestPostgresRecSaveFindAndUpsert(t *testing.T) {
...
func TestPostgresRecTenantScopeAndNotFound(t *testing.T) {
...
func TestPostgresCobRSaveFindList(t *testing.T) {
	store, _ := newRecStore(t)
	ctx := context.Background()
...
// TestPostgresRecurrenceClosedDBPropagatesErrors exercises every recurrence
...
func TestPostgresRecurrenceClosedDBPropagatesErrors(t *testing.T) {
	dsn := testDSN(t)
	db, err := postgres.Open(dsn)
	if err != nil {
		t.Fatalf("open: %v", err)
...
// TestPostgresFindRecByIDBadDevedorRow covers the devedor-rehydrate error branch: a
// row whose persisted devedor document is malformed cannot be reconstructed.
func TestPostgresFindRecByIDBadDevedorRow(t *testing.T) {
	store, dsn := newRecStore(t)
	ctx := context.Background()
...
// TestPostgresRecurrenceTransitionAtomicity proves the unit-of-work seam: a Rec
...
func TestPostgresRecurrenceTransitionAtomicity(t *testing.T) {
	store, dsn := newRecStore(t)
	ctx := context.Background()
	if err := store.SaveRec(ctx, mustRec(t, "RR1", "ten-1")); err != nil {
