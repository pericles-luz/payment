...
// TestClientProvisionLimiterThrottles: once the per-account bucket is empty the
// provisioning limiter answers 429 with Retry-After and the wrapped handler does
// not run.
func TestClientProvisionLimiterThrottles(t *testing.T) {
	t.Parallel()
	rl := newRateLimiter(1, 0, nil) // capacity 1, no refill
	var ran bool
	h := rl.middlewareClientProvision(60)(probe200(&ran))
...
// TestClientProvisionLimiterFailsOpen: with no account on the context (only
// reachable via a wiring bug placing the limiter before auth), the limiter admits
// the request rather than locking the Conta out of provisioning.
func TestClientProvisionLimiterFailsOpen(t *testing.T) {
	t.Parallel()
	rl := newRateLimiter(0, 0, nil) // a real key would be throttled immediately
	var ran bool
	h := rl.middlewareClientProvision(60)(probe200(&ran))

	r := httptest.NewRequest(http.MethodPost, "/v1/clients", nil) // no ctxAccountID
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, r)
