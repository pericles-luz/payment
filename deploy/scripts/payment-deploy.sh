...
# scp's upload runs as its own forced-command-exempt transfer to the fixed
# incoming path, while every interactive/remote-exec invocation lands here.
#
# Allowed verbs:
#   deploy     — atomically install the uploaded binary and restart the service.
...
readonly INCOMING="/opt/payment/incoming/payment-api"   # scp upload target
...
case "${verb}" in
  deploy)
    log "deploy: validating uploaded binary at ${INCOMING}"
    [ -f "${INCOMING}" ] || die "no uploaded binary at ${INCOMING} (did the ship step run?)"
    [ -s "${INCOMING}" ] || die "uploaded binary is empty"
