-- GERADO por scripts/gen-pg-migrations.py a partir de ../0012_bank_secret_vault.up.sql — NAO EDITE A MAO.
...
CREATE TABLE IF NOT EXISTS bank_credentials (
    ...
    secret_sealed       BYTEA NOT NULL, -- AES-256-GCM sealed OAuth secret; never plaintext
    creditor_key_sealed BYTEA NULL,     -- AES-256-GCM sealed PIX creditor key; NULL until set
...
CREATE TABLE IF NOT EXISTS bank_certificates (
    ...
    key_pem_sealed BYTEA NOT NULL, -- AES-256-GCM sealed mTLS private key; WRITE-ONLY, never read back
    updated_at    TEXT NOT NULL, -- RFC3339-UTC last-write instant
    PRIMARY KEY (tenant_id, bank_id)
);
